...
class UnderwaterWSN:
    ...
    def plot_nodes(self, layer_heights):
        ...
        for height in layer_heights:
            ...
            ax.plot_surface(x_plane, y_plane, z_plane, alpha=0.3, color='olive')
        ...
        ax.scatter(sink_x_vals, sink_y_vals, sink_z_vals, c='olive', marker='^', label='Sink Nodes')
    ...
    def plot_clusters(self, layer_number=None):
        ...
        for sink in self.sinks:
            ax.scatter(sink.x, sink.y, sink.z, c='olive', marker='^', label=f'Sink {sink.sink_id}')
