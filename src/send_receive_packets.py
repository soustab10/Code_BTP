...
def start(
    ...
    PacketSize = (
        my_model.hello_packet_len
        if packet_type == "Hello"
        else my_model.data_packet_len
    )
    ...
    for sender in senders:
        for receiver in receivers:
            ...
            distance = sqrt(
                ...
                + pow(sensors[sender.id].zd - sensors[receiver.id].zd, 2)
            )
            # print(f"dist b/w sender: {sender.id} and receiver: {receiver.id} is: {distance}")

            sensors[sender.id].E -= (
                (
                    my_model.ETX * PacketSize
                    + my_model.Emp * PacketSize * pow(distance, 4)
                )
                + ((my_model.p_t * PacketSize) / (my_model.bandwidth * htt))
            ) * my_model.NumPacket
    ...
    for receiver in receivers:
        ...
        sensors[receiver.id].E -= (
            (my_model.ERX + my_model.EDA) * PacketSize
            + ((my_model.p_r * PacketSize) / (my_model.bandwidth * htt))
        ) * my_model.NumPacket
